...
const quizData = {
  questions: [
    ...
    {
      type: "drawing",
      question: "Draw the script for 'NYA' (ᨎ)",
      targetAksara: "nya",
    },
    {
      type: "multiple-choice",
      question: "Which one is the script for 'PA'?",
      options: ["ᨄ", "ᨅ", "ᨆ", "ᨇ"],
      correct: 0,
    },
    {
      type: "drawing",
      question: "Draw the script for 'DA' (ᨉ)",
      targetAksara: "da",
    },
    {
      type: "multiple-choice",
      question: "Choose the appropriate script for 'TA'",
      options: ["ᨈ", "ᨉ", "ᨊ", "ᨋ"],
      correct: 0,
    },
    {
      type: "multiple-choice",
      question: "What is the script for the letter 'BA'?",
      options: ["ᨅ", "ᨌ", "ᨍ", "ᨎ"],
      correct: 0,
    },
  ],
...
export default function MatchmakingPage() {
  ...
  const [opponentFinishTime, setOpponentFinishTime] = useState<number | null>(
    null
  );
  ...
  const submitPlayerAnswer = (answerIndex?: number) => {
    ...
    setTimeout(() => {
      if (
        currentQuestionData.type === "multiple-choice" &&
        answerIndex === currentQuestionData.correct
      ) {
  ...
  // --- Game Logic ---
  useEffect(() => {
    if (gameState !== "playing" || opponentCurrentQuestion >= totalQuestions)
      return;
    ...
    const thinkingTimer = setTimeout(() => {
      ...
      if (
        newOpponentProgress >= totalQuestions &&
  ...
  useEffect(() => {
    if (gameState === "finished") {
      const calculatePerformanceScore = (
        ...
        const timeTaken =
          finishTime !== null ? INITIAL_TIME - finishTime : INITIAL_TIME;
        return (Math.pow(score, 2) * progress * 1000) / Math.max(timeTaken, 1);
      };
      const playerPerf = calculatePerformanceScore(
        playerScore,
        playerProgress,
        playerFinishTime
      );
      const opponentPerf = calculatePerformanceScore(
        opponentScore,
        opponentProgress,
        opponentFinishTime
      );
            ...
            <Card className="max-w-md mx-auto border-orange-200 shadow-lg">
              <CardContent className="p-8">
                ...
                <p className="text-gray-600 mb-6">
                  Challenge another player in a fast-paced script quiz! Who is
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Progress
                          value={(playerProgress / totalQuestions) * 100}
                ...
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <Progress
                          value={(opponentProgress / totalQuestions) * 100}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {currentQuestionData.options?.map((option, index) => (
                      <Button
                        ...
                        onClick={() => submitPlayerAnswer(index)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 text-left">
                  ...
                  >
                    ...
                    {playerFinishTime !== null ? (
                      <div className="text-xs text-gray-600 flex items-center">
                        ...
                        Finished in {INITIAL_TIME - playerFinishTime}s
                      </div>
                    ) : (
                    ...
                    <div className="text-sm font-bold text-gray-800 flex items-center">
                      <BarChart2 className="w-4 h-4 mr-1 text-blue-600" />
                      Perf. Score: {playerPerformanceScore.toLocaleString()}
                    </div>
                  </div>
                  <div
                    className={`p-4 rounded-lg space-y-2 ${
                  ...
                  >
                    ...
                    {opponentFinishTime !== null ? (
                      <div className="text-xs text-gray-600 flex items-center">
                        <Clock className="w-3 h-3 mr-1" />
                        Finished in {INITIAL_TIME - opponentFinishTime}s
                      </div>
                    ) : (
                      <div className="text-xs text-gray-500 mt-2">
                        Did not finish
                    ...
                    <div className="text-sm font-bold text-gray-800 flex items-center">
                      <BarChart2 className="w-4 h-4 mr-1 text-red-600" />
                      Perf. Score: {opponentPerformanceScore.toLocaleString()}
                    </div>
                  </div>
                </div>
